...
export default {
  "peter-jerry-ye:canvas/Canvas2D": {
    ...
    /** @type { (canvas: CanvasRenderingContext2D, x: number, y: number, width: number, height: number) => void} */
    clearRect: (canvas, x, y, width, height) => {
    ...
    set_stroke_style: (canvas, style) => {
      canvas.strokeStyle = style;
    },
    /** @type { (canvas: CanvasRenderingContext2D, style: string | CanvasGradient | CanvasPattern) => void} */
    set_fill_style: (canvas, style) => {
      canvas.fillStyle = style;
    },
    /** @type { (canvas: CanvasRenderingContext2D, font: string) => void} */
    set_font: (canvas, font) => {
      canvas.font = font;
    },
    ...
    drawImage_dim: (canvas, image, dx, dy, dWidth, dHeight) => {
    ...
    /** @type { (canvas: CanvasRenderingContext2D, sx: number, sy: number, sw: number, sh: number) => ImageData} */
    getImageData: (canvas, sx, sy, sw, sh) => {
      return canvas.getImageData(sx, sy, sw, sh);
    },
  },
  ...
  "peter-jerry-ye:canvas/String": {
    ...
    load: (offset, length) => {
      const bytes = new Uint16Array(
        globalThis["peter-jerry-ye:canvas"].memory.buffer,
        offset,
        length
      );
  ...
  "peter-jerry-ye:canvas/ImageData": {
    /** @type { (width: number, height: number) => ImageData} */
    new: (width, height) => new ImageData(width, height),
    /** @type { (imageData: ImageData, data: Uint8ClampedArray) => void} */
    setData: (imageData, data) => {
      imageData.data.set(data);
    },
    /** @type { (imageData: ImageData) => Uint8ClampedArray} */
    getData: (imageData) => imageData.data,
  },
  "peter-jerry-ye:canvas/Array": {
    ...
    /** @type {<T extends number>(array: Array<T>, offset: number) => void} */
    store_uint8_clamped_array: (array, offset) => {
      const view = new DataView(
        globalThis["peter-jerry-ye:canvas"].memory.buffer,
        offset
      );
  ...
  "peter-jerry-ye:canvas/event": {
    ...
    set_onkeydown: (callback) => {
      (
        globalThis["peter-jerry-ye:canvas"].eventTarget ?? globalThis
      ).onkeydown = callback;
    },
    /** @type {(callback: (_: KeyboardEvent) => void) => void} */
    set_onkeyup: (callback) => {
      (globalThis["peter-jerry-ye:canvas"].eventTarget ?? globalThis).onkeyup =
        callback;
    },
    /** @type {(callback: (_: MouseEvent) => void) => void} */
    set_onwheel: (callback) => {
      (globalThis["peter-jerry-ye:canvas"].eventTarget ?? globalThis).onwheel =
        callback;
    },
    /** @type {(callback: (_: MouseEvent) => void) => void} */
    set_onmousemove: (callback) => {
      (
        globalThis["peter-jerry-ye:canvas"].eventTarget ?? globalThis
      ).onmousemove = callback;
    },
    /** @type {(callback: (_: MouseEvent) => void) => void} */
    set_onmouseenter: (callback) => {
      (
        globalThis["peter-jerry-ye:canvas"].eventTarget ?? globalThis
      ).onmouseenter = callback;
    },
    /** @type {(callback: (_: MouseEvent) => void) => void} */
    set_onmousedown: (callback) => {
      (
        globalThis["peter-jerry-ye:canvas"].eventTarget ?? globalThis
